...
FIGMA_TOKENS = re.compile(
    ...
    r"|StatementAssessmentQuestion|TransitionScreen|HotSpot|DragDrop|MatchingQuestion|גרור[- ]?הדבק"
)
FIGMA_NAME = re.compile(r"(?:שם המסך\s*)?בפיגמה\s*[:\-–]\s*([^\n|]*)")
tmpl_stats = {"תבנית": 0, "לא-בתבנית": 0, "ללא-ציון": 0}
...
for sl, body in slides:
    ...
    # #11 מקור-תבנית (פיגמה) + הערות הפקה — שני מקרים אמינים בלבד (לא מנחשים תבנית פר-שקף)
    notes_len = len(notes.strip())
    nm = FIGMA_NAME.search(body)
    name_val = nm.group(1).strip() if nm else ""
    placeholder_name = name_val in ("", "אין", "-") or name_val.startswith("<")
    has_token = bool(FIGMA_TOKENS.search(body))
    if "לא בתבנית" in visible:
        tmpl_stats["לא-בתבנית"] += 1
        if notes_len < 10:
            add(sl, "#11 'לא בתבנית' ללא הערת הפקה", "הערות דובר ריקות/חסרות")
    elif (nm and not placeholder_name) or has_token:
        tmpl_stats["תבנית"] += 1
    else:
        tmpl_stats["ללא-ציון"] += 1
    # שם-תבנית שהוצהר אך ריק/פלייסהולדר, ואין הערות → דגל אמין
    if nm and placeholder_name and not has_token and notes_len < 25 and "לא בתבנית" not in visible:
        add(sl, "#11 שם תבנית-פיגמה ריק/חסר", f'"בפיגמה: {name_val or "(ריק)"}" + אין הערת הפקה')
...
print(f"מקור-תבנית (#11): {tmpl_stats['תבנית']} מצהירים על תבנית-פיגמה · {tmpl_stats['לא-בתבנית']} 'לא בתבנית' · {tmpl_stats['ללא-ציון']} ללא-ציון (לרוב שקפי-מצב של תבנית)")
if ans:
